...
def build_agents(repo_root: Path, game_knowledge: Optional[KnowledgeProtocol] = None) -> dict[str, Agent]:
    ...
    storytelling = Agent(
        ...
        description="Narrative, pacing, branching, PT-BR player-facing prose for the campaign under REPO_ROOT.",
        role="Narrative, pacing, branching, PT-BR player-facing prose for the primary campaign in REPO_ROOT",
        model=model,
